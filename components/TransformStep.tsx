...
const TransformStep = (props: TransformStepProps) => {
  ...
  return (
    <div
      className={`border-2 border-${color}-500 border-opacity-50 shadow-xl shadow-${color}-800/60 rounded-md`}
    >
      <div
        className={`text-sm text-gray-200  px-6 py-1 bg-${color}-500 bg-opacity-50 flex justify-between items-center rounded-sm`}
      >
      ...
      {!value && emptyMessage && (
        <div
          className="text-gray-400 text-xs h-full flex items-center text-center"
          style={{ height: "500px" }}
        >
